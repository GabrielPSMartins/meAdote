...
class Etapa3Page extends StatelessWidget {
  final Animal animal;
  Etapa3Page({required this.animal});
    ...
    return Scaffold(
      ...
      body: SafeArea(child: Padding(padding: EdgeInsets.all(16), child: Column(children: [
        ...
        Expanded(child: ListView(children: [
          Text('Termo de Compromisso', style: TextStyle(fontWeight: FontWeight.bold)),
          SizedBox(height:8),
          CheckboxListTile(value: false, onChanged: (_) {}, title: Text('Comprometo-me a cuidar do animal')),
          CheckboxListTile(value: false, onChanged: (_) {}, title: Text('Autorizo verificações')),
          SizedBox(height:12),
          Row(children: [Expanded(child: OutlinedButton(onPressed: () => Navigator.pop(context), child: Text('Voltar'))), SizedBox(width:8), Expanded(child: ElevatedButton(onPressed: () { Navigator.popUntil(context, (route) => route.isFirst); ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Solicitação enviada (simulado)'))); }, child: Text('Enviar solicitação'), style: ElevatedButton.styleFrom(backgroundColor: Theme.of(context).elevatedButtonTheme.style?.backgroundColor?.resolve({}) ?? Color(0xFFA0A0A0), foregroundColor: Colors.black)))]),
          SizedBox(height:8),
          Text('Etapa 4 de 4', textAlign: TextAlign.center, style: TextStyle(color: Color(0xFFB3B3B3))),
        ]))
      ]))),
    );
  }
